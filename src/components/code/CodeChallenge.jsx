    ...
    return (
        <div className="px-8 md:px-16 py-10 md:p-16">
            ...
            <div className="flex items-center justify-between mb-12">
                <h2 className="text-[#f0ebe2] font-black text-[2rem] md:text-[3rem] lg:text-[4rem] leading-tight tracking-tighter uppercase">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {challenges.map((ch) => {
                    ...
                    const colors = colorsDiff[ch.difficulty];

                    return (
                        <div key={ch.id} className={`group p-6 border rounded-xl transition-all duration-500 relative overflow-hidden ${isSolved ? 'border-[#378079]/40 bg-[#378079]/5' : 'border-white/5 bg-[#0d1117] hover:border-white/20'}`}>
                            ...
                            <div className="flex items-start justify-between mb-6">
                                <span className="text-[8px] font-black uppercase tracking-widest px-2 py-1 border rounded-sm" style={{ color: colors.main, borderColor: colors.border, backgroundColor: colors.bg }}>
                                    {ch.difficulty}
                                </span>
                            ...
                            <button onClick={() => toggleSolve(ch.id)} className={`w-full py-3 text-[9px] font-black uppercase tracking-widest border transition-all duration-300 active:scale-[0.98]    ${isSolved ? 'bg-[#378079] border-[#378079] text-[#06090f]' : 'border-white/10 text-white/30 hover:border-[#378079] hover:text-[#378079]'}`}>
