        ...
        <section className="px-8 md:px-16 py-10 border-t border-white/5">
            ...
            <form onSubmit={handleSubmit} className="flex gap-3 mb-4 max-w-xl">
                <input
                    ...
                    placeholder="MÜƏLLİF ADI (İNGİLİSCƏ)..."
                    className="custom-input flex-1"
                />
                <button
                    type="submit"
                    disabled={loading}
                    className="beveled-box px-6 py-3 border border-white text-[10px] font-black tracking-[0.3em] uppercase hover:bg-white hover:text-black transition-all disabled:opacity-40"
                >
                    {loading ? '...' : 'AXTAR'}
                </button>
            </form>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
                {popular.map(p => (
                    <button
                        key={p}
                        onClick={ () => { setSearch(p); fetchAuthor(p); }}
                        className="px-3 py-1.5 text-[9px] font-bold tracking-[0.15em] uppercase border border-white/10 text-white/30 hover:border-[#378079]/40 hover:text-[#378079] transition-all rounded-lg"
                    >
                        {p}
                    </button>
                ))}
                ...
                <div className="max-w-3xl bg-[#0b0f17] border border-white/5 rounded-2xl overflow-hidden">
                    <div className="flex flex-col md:flex-row gap-0">
                        ...
                        <div className="p-6 md:p-8">
                            ...
                            <p className="text-[13px] text-[#f0ebe2]/50 leading-relaxed">
                                {author.extract && author.extract.length > 400
                                    ? author.extract.slice(0, 400) + '...'
                                    : author.extract}
                            </p>
                            {author.url && (
                                <a
                                    ...
                                    className="inline-block mt-5 text-[10px] font-bold tracking-[0.2em] uppercase text-[#378079] hover:text-[#4dbdb1] transition-colors"
                                >
